...
# from OctopusArmConnections.OctopusArmWithNewPressureModel.contact_with_rigid_body import ExternalContactForMemoryBlock
# #from OctopusArmConnections.OctopusArmWithNewPressureModel.connect_with_sucker_force_rod_rigid_body import SuckerForcesConnectForMemoryBlock
# from OctopusArmConnections.OctopusArmWithNewPressureModel.connect_with_sucker_force_rod_rigid_body_with_friction import SuckerForcesConnectForMemoryBlock
# from OctopusArmConnections.OctopusArmWithNewPressureModel.contact_with_rigid_body_friction import ExternalContactWithFrictionForMemoryBlock
...
class Environment:
    ...
    def reset(self):
        ...
        target_start = (
            ...
            #            - (3.0*target_radius + self.straight_rod_list[6].radius[rod_idx] + 2*self.helical_rod_list[0].radius[0] + 2*self.ring_rod_list[0].radius[0])* (binormal+normal)/np.linalg.norm(binormal+normal) #0.5 * (outer_base_radius + target_radius) * binormal
            - (
                1.0 * target_radius
            ...
            * binormal  # 0.5 * (outer_base_radius + target_radius) * binormal
            - (
                2.7 * target_radius
                + self.straight_rod_list[6].radius[rod_idx]
                + 2 * self.helical_rod_list[0].radius[0]
            ...
            * normal  # 0.5 * (outer_base_radius + target_radius) * binormal
            + 0 * direction
        ...
        for idx, rod in enumerate(self.straight_rod_list):
            ...
            if idx == 4 or idx == 5 or idx == 6:
                ...
                # self.simulator.add_forcing_to(rod).using(
                #     LocalActivation, ramp_interval = 2.0, ramp_up_time=0.0, ramp_down_time = 8.0, start_idx = 0, end_idx = 170, activation_level=0.3,
                # )
        #            if idx == 1 or idx == 2 or idx == 8:
        #                self.simulator.add_forcing_to(rod).using(
        #                     ActivationRampUpRampDown,
        ...
        for idx_cylinder, cylinder in enumerate(self.cylinder_list):
            for idx, rod in enumerate(
                ...
                #     ExternalContactForMemoryBlock, k=5e2, nu=1.0#0.3
                # )
                self.simulator.connect(rod, cylinder).using(
