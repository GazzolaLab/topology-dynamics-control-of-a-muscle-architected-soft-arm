...
class Environment:
    def __init__(
        ...
        time_step = 0.98e-5  # 1.25e-6 * 2 * 2 #* 2  # * 2 * 2 #* 2# (
        #     2.0e-5 / 2 / 2 / 4
    ...
    def save_state(self, directory: str = "", time=0.0, verbose: bool = False):
        ...
        for idx, rod in enumerate(self.rod_list + self.cylinder_list):
    ...
    def reset(self):
        ...
        cylinder_start = (
            start
            + (1.5 + 0.0) * (outer_base_radius + cylinder_radius) * binormal
            + (125 + 0) * direction
        ...
        target_start = (
            start
            + 90 * binormal  # 0.5 * (outer_base_radius + target_radius) * binormal
            + (100) * direction
        ...
        for _, cylinder in enumerate(self.cylinder_list[:2]):
            self.simulator.constrain(cylinder).using(
        ...
        self.simulator.constrain(self.target_cylinder).using(
            FixPositionDirectorForSomeTime,
            constrained_position_idx=(0,),
            constrained_director_idx=(0,),
            release_time=4.5 - 0.5,
        )
        ...
        for _, rod in enumerate(self.cylinder_list[2:]):
            self.simulator.constrain(rod).using(
                ExponentialDampingBCRigidBodies, nu=50, time_step=self.time_step
            )
        ...
        for idx, rod in enumerate(self.straight_rod_list):
            ...
            if idx == 1 or idx == 2 or idx == 8:
                ...
                #                )
                #                self.simulator.add_forcing_to(rod).using(SigmoidActivationLongitudinalMuscles, start_time = 1, end_time = 8, start_idx = 0, end_idx=170, beta=1, tau=0.02, activation_level_max=0.20, activation_level_end=0.20)
                self.simulator.add_forcing_to(rod).using(
                    SigmoidActivationLongitudinalMuscles,
                    start_time=0,
                    end_time=0 + 40 * 0.03,
                    ...
                    beta=1,
                    tau=0.03,
                    activation_level_max=0.20,
                    activation_level_end=0.20,
                )
                self.simulator.add_forcing_to(rod).using(
                    SigmoidActivationLongitudinalMuscles,
                    start_time=0,
                ...
                self.simulator.add_forcing_to(rod).using(
                    ...
                    start_time=3.1 - 0.5,
                    end_time=15.0,
                    start_idx=115,  # 0, #30,#int(115/3),
                    end_idx=177,  # 60,#int(180/3),
                    beta=1,
                    tau=0.02,
                    activation_level_max=0.30,  # 0.035
                    activation_level_end=0.30,  # 0.035
                )

            self.simulator.add_forcing_to(rod).using(
                SigmoidActivationLongitudinalMuscles,
                start_time=7.1 - 2.0 - 0.2,
                end_time=9.0,
                start_idx=0,
                end_idx=100,
                beta=1,
                tau=0.01,
                activation_level_max=0.25,
        ...
        for idx_cylinder, cylinder in enumerate(self.cylinder_list[:2]):
            for idx, rod in enumerate(
                self.helical_rod_list
                #                 +
                #                 self.straight_rod_list+
                #                 self.ring_rod_list
                #                [self.straight_rod_list[0] , self.straight_rod_list[1],  self.straight_rod_list[2],  self.straight_rod_list[8] ]
            ):  # (self.straight_rod_list[1:]):
                #                self.simulator.connect(rod, cylinder).using(
                #                    ExternalContactForMemoryBlock, k=2e2, nu=1.0
                #                )
                self.simulator.connect(rod, cylinder).using(
                    ExternalContactWithFrictionForMemoryBlock,
                    k=1e4,
                    nu=2.0,
                    velocity_damping_coefficient=1e5 * 0,
                    friction_coefficient=0.1 * 0,
                )
        ...
        # Define contact between target and obstacles
        for idx_cylinder, cylinder in enumerate(self.cylinder_list[:2]):
            self.simulator.connect(self.target_cylinder, cylinder).using(
                ExternalContactCylinderCylinderForMemoryBlock, k=1e3, nu=1.0
            )

        #        # Sucker forces and activations
        #        self.activation_array_block = np.zeros((len(self.helical_rod_list), n_elem_helical_rod))
        #
        ...
        ##
        ##            self.simulator.add_forcing_to(rod).using(SigmoidSuckerActivation, start_time=3,
        ##                                                                     activation_array = self.sucker_activation_array_list[idx] ,
        ##                                                     end_time=3+100*0.002, start_idx=100, end_idx=350,
        ...
        ##                self.simulator.connect(rod, cylinder).using(
        ##                    SuckerForcesConnectForMemoryBlock, k=1E4,  sucker_threshold=0.1, sucker_activation=self.activation_array_block,
