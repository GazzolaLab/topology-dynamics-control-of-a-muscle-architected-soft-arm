...
class Environment:
    ...
    def reset(self):
        ...
        for idx, rod in enumerate(self.straight_rod_list):
            ...
            #                 self.simulator.add_forcing_to(rod).using(
            #                     LocalActivation, ramp_interval = 4.0, ramp_up_time=1.5, ramp_down_time = 220.0, start_idx = 0, end_idx = 178, activation_level=0.3,
            #                 )
            if idx >= 1:
                start_idx = int((idx - 1) * 20)
                end_idx = start_idx + 40
                self.simulator.add_forcing_to(rod).using(
                    LocalActivation,
                    ramp_interval=1.0,
                    ramp_up_time=0.0 + (idx - 1) * 0.8,
                    ramp_down_time=150.0,
                    start_idx=start_idx,
                    end_idx=end_idx,
                    activation_level=1.0,  # * (9 - idx) / 8,
                )
        #            if idx == 1 or idx == 2 or idx == 8:
        #                self.simulator.add_forcing_to(rod).using(
        #                     ActivationRampUpRampDown,
        ...
        #     if idx < 60:
        #         self.simulator.add_forcing_to(rod).using(SigmoidActivationTransverseMuscles, rod_idx = idx, start_time = 7.0, end_time=15, beta=1, tau=0.01, activation_level_max=0.4, activation_level_end=0.4) #1.0
        #     elif idx>=60 and idx <=160:
        #         self.simulator.add_forcing_to(rod).using(SigmoidActivationTransverseMuscles, rod_idx = idx, start_time = 7.0, end_time=15, beta=1, tau=0.01, activation_level_max=0.4, activation_level_end=0.40) #0.5
        #     else:
        #         self.simulator.add_forcing_to(rod).using(SigmoidActivationTransverseMuscles, rod_idx = idx, start_time = 7.0, end_time=15, beta=1, tau=0.01, activation_level_max=0.03, activation_level_end=0.03) #0.5
        ...
        #
        # for idx, rod in enumerate(self.helical_rod_list[4:]):
        #     self.simulator.add_forcing_to(rod).using(LocalActivation, ramp_interval = 0.5, ramp_up_time=0.0, ramp_down_time = 3.0, start_idx = 0, end_idx =356, activation_level=0.3,)
        #
        # for idx, rod in enumerate(self.helical_rod_list[:4]):
        #     self.simulator.add_forcing_to(rod).using(LocalActivation, ramp_interval = 1.0, ramp_up_time=4.1, ramp_down_time = 120.5, start_idx = 0, end_idx =356, activation_level=0.3,)
        #
